...
void float4x4::CreateFromQuaternion__ref0_out1(::float4& quaternion, ::float4x4& result)
{
...
void float4x4::CreateFromYawPitchRoll__out3(float yaw, float pitch, float roll, ::float4x4& result)
{
::float4 quaternion;
float4::CreateFromYawPitchRoll__out3(yaw, pitch, roll, quaternion);
float4x4::CreateFromQuaternion__ref0_out1(quaternion, result);
}

void float4x4::CreateLookAt__ref0_ref1_ref2_out3(::float3& cameraPosition, ::float3& cameraTarget, ::float3& cameraUpVector, ::float4x4& result)
{
...
void float4x4::CreateOrthographicOffCenter__out6(float left, float right, float bottom, float top, float zNearPlane, float zFarPlane, ::float4x4& result)
{
...
void float4x4::CreatePerspectiveFieldOfView__out4(float fieldOfView, float aspectRatio, float nearPlaneDistance, float farPlaneDistance, ::float4x4& result)
{
...
void float4x4::CreateScale__out1(float scale, ::float4x4& result)
{
float4x4::CreateScale__out3(scale, scale, scale, result);
}

void float4x4::CreateScale__out3(float xScale, float yScale, float zScale, ::float4x4& result)
{
result.M11 = xScale;
result.M12 = 0;
result.M13 = 0;
...
void float4x4::CreateTranslation__out3(float x, float y, float z, ::float4x4& result)
{
result.M11 = 1;
result.M12 = 0;
result.M13 = 0;
...
void float4x4::CreateTranslation__ref0_out1(::float3& position, ::float4x4& result)
{
result.M11 = 1;
result.M12 = 0;
result.M13 = 0;
...
void float4x4::InverseTranspose__ref0_out1(::float4x4& matrix, ::float4x4& result)
{
::float4x4 inverted;
    if (!float4x4::TryInvert__ref0_out1(matrix, inverted))
    {
throw new InvalidOperationException("Cannot invert a non-invertible transform matrix.");
    }
float4x4::Transpose__ref0_out1(inverted, result);
}

void float4x4::Multiply__ref0_ref1_out2(::float4x4& matrix1, ::float4x4& matrix2, ::float4x4& result)
{
...
void float4x4::Transpose__ref0_out1(::float4x4& matrix, ::float4x4& result)
{
::float4x4 ret;
ret.M11 = matrix.M11;
ret.M12 = matrix.M21;
...
bool float4x4::TryInvert__ref0_out1(::float4x4& matrix, ::float4x4& result)
{
const double m0 = matrix.M11;
const double m1 = matrix.M12;
const double m2 = matrix.M13;
const double m3 = matrix.M14;
const double m4 = matrix.M21;
const double m5 = matrix.M22;
const double m6 = matrix.M23;
const double m7 = matrix.M24;
const double m8 = matrix.M31;
const double m9 = matrix.M32;
const double m10 = matrix.M33;
const double m11 = matrix.M34;
const double m12 = matrix.M41;
const double m13 = matrix.M42;
const double m14 = matrix.M43;
const double m15 = matrix.M44;
const double i0 = m5 * m10 * m15 - m5 * m11 * m14 - m9 * m6 * m15 + m9 * m7 * m14 + m13 * m6 * m11 - m13 * m7 * m10;
const double i4 = -m4 * m10 * m15 + m4 * m11 * m14 + m8 * m6 * m15 - m8 * m7 * m14 - m12 * m6 * m11 + m12 * m7 * m10;
const double i8 = m4 * m9 * m15 - m4 * m11 * m13 - m8 * m5 * m15 + m8 * m7 * m13 + m12 * m5 * m11 - m12 * m7 * m9;
const double i12 = -m4 * m9 * m14 + m4 * m10 * m13 + m8 * m5 * m14 - m8 * m6 * m13 - m12 * m5 * m10 + m12 * m6 * m9;
const double determinant = m0 * i0 + m1 * i4 + m2 * i8 + m3 * i12;
    if (Math::Abs(determinant) <= 0.0)
    {
result = ::float4x4();
return false;    }
const double i1 = -m1 * m10 * m15 + m1 * m11 * m14 + m9 * m2 * m15 - m9 * m3 * m14 - m13 * m2 * m11 + m13 * m3 * m10;
const double i5 = m0 * m10 * m15 - m0 * m11 * m14 - m8 * m2 * m15 + m8 * m3 * m14 + m12 * m2 * m11 - m12 * m3 * m10;
const double i9 = -m0 * m9 * m15 + m0 * m11 * m13 + m8 * m1 * m15 - m8 * m3 * m13 - m12 * m1 * m11 + m12 * m3 * m9;
const double i13 = m0 * m9 * m14 - m0 * m10 * m13 - m8 * m1 * m14 + m8 * m2 * m13 + m12 * m1 * m10 - m12 * m2 * m9;
const double i2 = m1 * m6 * m15 - m1 * m7 * m14 - m5 * m2 * m15 + m5 * m3 * m14 + m13 * m2 * m7 - m13 * m3 * m6;
const double i6 = -m0 * m6 * m15 + m0 * m7 * m14 + m4 * m2 * m15 - m4 * m3 * m14 - m12 * m2 * m7 + m12 * m3 * m6;
const double i10 = m0 * m5 * m15 - m0 * m7 * m13 - m4 * m1 * m15 + m4 * m3 * m13 + m12 * m1 * m7 - m12 * m3 * m5;
const double i14 = -m0 * m5 * m14 + m0 * m6 * m13 + m4 * m1 * m14 - m4 * m2 * m13 - m12 * m1 * m6 + m12 * m2 * m5;
const double i3 = -m1 * m6 * m11 + m1 * m7 * m10 + m5 * m2 * m11 - m5 * m3 * m10 - m9 * m2 * m7 + m9 * m3 * m6;
const double i7 = m0 * m6 * m11 - m0 * m7 * m10 - m4 * m2 * m11 + m4 * m3 * m10 + m8 * m2 * m7 - m8 * m3 * m6;
const double i11 = -m0 * m5 * m11 + m0 * m7 * m9 + m4 * m1 * m11 - m4 * m3 * m9 - m8 * m1 * m7 + m8 * m3 * m5;
const double i15 = m0 * m5 * m10 - m0 * m6 * m9 - m4 * m1 * m10 + m4 * m2 * m9 + m8 * m1 * m6 - m8 * m2 * m5;
const double inverseDeterminant = 1.0 / determinant;
result.M11 = static_cast<float>((i0 * inverseDeterminant));
result.M12 = static_cast<float>((i1 * inverseDeterminant));
result.M13 = static_cast<float>((i2 * inverseDeterminant));
result.M14 = static_cast<float>((i3 * inverseDeterminant));
result.M21 = static_cast<float>((i4 * inverseDeterminant));
result.M22 = static_cast<float>((i5 * inverseDeterminant));
result.M23 = static_cast<float>((i6 * inverseDeterminant));
result.M24 = static_cast<float>((i7 * inverseDeterminant));
result.M31 = static_cast<float>((i8 * inverseDeterminant));
result.M32 = static_cast<float>((i9 * inverseDeterminant));
result.M33 = static_cast<float>((i10 * inverseDeterminant));
result.M34 = static_cast<float>((i11 * inverseDeterminant));
result.M41 = static_cast<float>((i12 * inverseDeterminant));
result.M42 = static_cast<float>((i13 * inverseDeterminant));
result.M43 = static_cast<float>((i14 * inverseDeterminant));
result.M44 = static_cast<float>((i15 * inverseDeterminant));
return true;}
