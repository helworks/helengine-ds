...
/// <summary>
/// Imports source textures and source fonts through the shared editor import contracts for Nintendo DS builder-owned cook work items.
/// </summary>
public sealed class NintendoDsPlatformCookSourceProcessor : INintendoDsPlatformCookSourceProcessor {
    ...
    /// <summary>
    /// Shared texture processor used to apply editor-resolved Nintendo DS texture settings.
    /// </summary>
    ...
    IReadOnlyList<TextureImporterRegistration> GetTextureImporterRegistrations() {
        ...
        List<TextureImporterRegistration> textureRegistrations = [];
    ...
    Assembly LoadEditorWindowsAssembly() {
        ...
        if (!File.Exists(assemblyPath)) {
            throw new FileNotFoundException(
                "Windows editor backend assembly 'helengine.editor.windows' was not found. Build helengine.editor.windows before executing Nintendo DS builder-owned cook work items.",
                assemblyPath);
        }
    ...
    static string ResolveHelengineRootPath() {
        ...
        string configuredNintendoDsRepositoryRootPath = Environment.GetEnvironmentVariable("HELENGINE_DS_REPOSITORY_ROOT") ?? string.Empty;
        if (!string.IsNullOrWhiteSpace(configuredNintendoDsRepositoryRootPath)) {
            string siblingHelengineRootPath = Path.GetFullPath(Path.Combine(configuredNintendoDsRepositoryRootPath, "..", "helengine"));
